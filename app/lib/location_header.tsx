  ...
  return (
    <div className={styles.home_content_column}>
        ...
        {weather ? <div className={styles.home_content_info}>
                <h4 className={styles.home_content_title_info}>Последнее обновление: {moment(weather.ts).format('HH:mm MM/DD/YYYY')}</h4>
                <div className={styles.home_content_title_info}>
                    ...
                    {Number(weather.pr) >= 1000 ? [1,2,3].map(n => <AiOutlineUser key={n}/>) : weather.pr >500 && weather.pr < 1000 ? [1,2].map(n => <AiOutlineUser key={n}/>): <AiOutlineUser/> }
